...
data "aws_kms_alias" "sqs" {
  count = var.deploy_sqs && var.encrypted_sqs ? 1 : 0

  name = "alias/${var.kms_master_key_alias}"
}
...
resource "aws_sqs_queue" "queue" {
  ...
  kms_master_key_id                 = var.encrypted_sqs != false ? data.aws_kms_alias.sqs[0].id : null
